...
/**
 * Prisma Enum reference (do NOT use raw strings for these):
 *
 *   Sex         : male | female
 *
 *   Civil       : single | married | widow | seperated | annulled | co_habitation
 *                 NOTE: "seperated" is the schema spelling (typo preserved intentionally)
 *
 *   Emp_Status  : student | employed | retired | unemployed | unknown
 *
 *   E_attainment: none | elementary | highschool | vocational | college | postgraduate
 *
 * Encrypted fields (must decryptAll before reading):
 *   f_name, m_name, l_name, s_name, b_place, house_no,
 *   email_address, contact_no, citizenship, occupation
 *
 * Non-encrypted fields (safe to read after decryptAll):
 *   sex (Sex enum), b_date, emp_status (Emp_Status enum),
 *   civil_status (Civil enum), sector (plain String),
 *   remarks, purok_id, blood_type, education (E_attainment enum)
 */
